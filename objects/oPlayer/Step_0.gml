...
var _graze_radius = 15;
var _reset_radius = 50;

if (launching)
{
    launch_timer++;

    var t = launch_timer / launch_time;
    t = clamp(t, 0, 1);

    var e = 1 - power(1 - t, 3);

    y = lerp(launch_start_y, launch_target_y, e);
...
if (parry_enabled) {
	if (!instance_exists(oParryCooldown))
	{
	    instance_create_layer(x, y, "Instances", oParryCooldown);
	}
	if (keyboard_check_pressed(ord("Z")) && parry_cooldown <= 0) {
	    parry_timer = parry_window;
	    parry_success = false;
		var fx = instance_create_layer(x, y, "Instances", oParry);
			fx.facing = facing
	}
	if parry_timer > 0 {
		parry_timer--
	
		if parry_timer == 0 && !parry_success {
			parry_cooldown = parry_failed_cooldown
		}
	}
	if parry_cooldown > 0 {
		parry_cooldown --
	...
	if (room == rMainHub)
	{
	    ...
	    {
	        show_parry_range = !show_parry_range;
	    }
	}
	else
	{
	    show_parry_range = false;
	}
} else {
	parry_timer = 0;
	parry_cooldown = 0;
	parry_success = false;
	show_parry_range = false;
	if (instance_exists(oParryCooldown)) with (oParryCooldown) instance_destroy();
}
