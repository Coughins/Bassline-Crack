...
if (chain_mode)
{
    ...
    if (chain_fork_timer <= 6 && chain_fork_timer >= 0)
    {
        var _tele_p = 1 - (chain_fork_timer / 6);
        image_blend = merge_color(c_white, make_color_rgb(120, 200, 255), 1 - _tele_p * 0.7);
    }

    if (chain_fork_timer <= 0)
    {
        if (chain_generation < chain_max_generation)
        {
            var _spread = 26;
            for (var _side = -1; _side <= 1; _side += 2)
            {
                var _child_dir = direction + _side * (_spread + random_range(-6, 6));

                if (instance_exists(oPlayer))
                {
                    var _to_player = point_direction(x, y, oPlayer.x, oPlayer.y);
                    var _diff = angle_difference(_child_dir, _to_player);
                    var _danger_cone = 22;
                    if (abs(_diff) < _danger_cone)
                    {
                        var _sign = (_diff >= 0) ? 1 : -1;
                        _child_dir = _to_player + _sign * (_danger_cone + 10);
                    }
                }

                var _child = instance_create_layer(x, y, layer, oHalfCircleBurst);
                _child.direction = _child_dir;
                _child.speed = speed * random_range(0.92, 1.08);
                _child._size = max(0.5, _size * 0.82);
                _child.image_alpha = 1;
                _child.chain_mode = true;
                _child.chain_generation = chain_generation + 1;
                _child.chain_max_generation = chain_max_generation;
                _child.chain_fork_timer = irandom_range(14, 20);
            }
        }
        instance_destroy();
    }
...
scr_add_light(x, y, make_color_rgb(120, 200, 255), 1.5);
