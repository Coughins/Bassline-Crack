...
for (var i = 0; i < 3; i++)
{
	array_push(menu_cracks_top, {
		pos: random_range(0.15, 0.85),
		width: random_range(0.01, 0.015)
	});

	array_push(menu_cracks_bottom, {
		pos: random_range(0.15, 0.85),
		width: random_range(0.01, 0.015)
	});
}
...
for (var i = 0; i < 6; i++)
{
	array_push(menu_cracks_middle, {
		line: irandom(11),
		pos: random_range(0.2, 0.8),
		width: random_range(0.005, 0.01)
	});
}

audio_stop_all()

if (!variable_global_exists("startup_notice_seen")) global.startup_notice_seen = false

startup_notice_screen = !global.startup_notice_seen
startup_notice_age = 0
startup_notice_exit = 0
startup_notice_exit_length = 24

if (startup_notice_screen) global.startup_notice_seen = true

title_screen = true
title_age = 0
title_exit = 0
title_exit_length = 34
title_input_wait_release = startup_notice_screen
menu_age = 0
menu_reveal = 0
menu_flash = 0
...
menu[MENU_SUB_KEYBOARD] = "KEYBOARD"
menu[MENU_SUB_GAMEPAD] = "GAMEPAD"
...
option[MENU_SUB_SETTINGS][5] = "Set defaults"
...
for (var i = 0; i < menu_number; i++)
	option_width_max[i] = array_get_max_string_width(option[i]) + 100;
